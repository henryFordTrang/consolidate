...
export default {
  ...
  methods: {
    ...
    // 滑动
    touchMove (event) {
      const touchPageXMove = event.pageX || event.changedTouches[0].pageX
      const touchPageYMove = event.pageY || event.changedTouches[0].pageY
      const viewScrollTop = this.$refs.pageScroll.scrollTop
      const viewOffsetHeight = this.$refs.pageScroll.offsetHeight
      const contentScrollHeight = this.$refs.pageScrollContent.scrollHeight
